...
public class BlogController {
    ...
    public ResultUtil list(@RequestParam Map<String, Object> params) {
        ...
        return ResultGeneratorUtil.genSuccessResult(blogService.getBlogPage(pageUtil));
    }
    ...
    @ResponseBody
    public ResultUtil save(@RequestParam("blogTitle") String blogTitle, @RequestParam(name = "blogSubUrl", required = false) String blogSubUrl, @RequestParam("blogCategoryId") Integer blogCategoryId, @RequestParam("blogTags") String blogTags, @RequestParam("blogContent") String blogContent, @RequestParam("blogCoverImage") String blogCoverImage, @RequestParam("blogStatus") Byte blogStatus, @RequestParam("enableComment") Byte enableComment) {
        if (StringUtils.isEmpty(blogTitle)) {
            return ResultGeneratorUtil.genFailResult("请输入文章标题");
        }
        if (blogTitle.trim().length() > 150) {
            return ResultGeneratorUtil.genFailResult("标题过长");
        }
        if (StringUtils.isEmpty(blogTags)) {
            return ResultGeneratorUtil.genFailResult("请输入文章标签");
        }
        if (blogTags.trim().length() > 150) {
            return ResultGeneratorUtil.genFailResult("标签过长");
        }
        if (blogSubUrl.trim().length() > 150) {
            return ResultGeneratorUtil.genFailResult("路径过长");
        }
        if (StringUtils.isEmpty(blogContent)) {
            return ResultGeneratorUtil.genFailResult("请输入文章内容");
        }
        if (StringUtils.isEmpty(blogCoverImage)) {
            return ResultGeneratorUtil.genFailResult("封面图不能为空");
        }
        Blog blog = new Blog();
        ...
        if ("success".equals(saveBlogResult)) {
            ...
            return ResultGeneratorUtil.genFailResult(saveBlogResult);
        }
    }
    ...
    public ResultUtil update(@RequestParam("blogId") Long blogId, @RequestParam("blogTitle") String blogTitle, @RequestParam(name = "blogSubUrl", required = false) String blogSubUrl, @RequestParam("blogCategoryId") Integer blogCategoryId, @RequestParam("blogTags") String blogTags, @RequestParam("blogContent") String blogContent, @RequestParam("blogCoverImage") String blogCoverImage, @RequestParam("blogStatus") Byte blogStatus, @RequestParam("enableComment") Byte enableComment) {
        ...
        if ("success".equals(updateBlogResult)) {
            ...
            return ResultGeneratorUtil.genFailResult(updateBlogResult);
        }
    }
